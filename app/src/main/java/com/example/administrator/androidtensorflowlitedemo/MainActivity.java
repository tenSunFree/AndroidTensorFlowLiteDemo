...
public class MainActivity extends AppCompatActivity {
            ...
            public void onImage(CameraKitImage cameraKitImage) {
                ...
                for (int i = 0; i < resultSplitStrings.length; i++) {
                    if (resultSplitStrings.length == 3) {
                        ...
                        Toast.makeText(MainActivity.this, "什麼!? 判斷到的數量居然超過3個, 請與作者聯繫, 謝謝!!", Toast.LENGTH_SHORT).show();
                    }
                }
    ...
    protected void onResume() {
        super.onResume();
        cameraView.start();                                                                         // 請求權限並打開拍照功能
    }

    @Override
    protected void onPause() {
        cameraView.stop();                                                                          // 關閉拍照功能
        super.onPause();
    }
            ...
            public void run() {
                try {
                    classifier = TensorFlowImageClassifier.create(
                            getAssets(),                                                            // 取得AssetManager, 為了读取assets目录下的文件
                            MODEL_PATH,
